...
const vec3 FogColor = vec3(1.6, 0.50, 0.00);
...
vec2 hash( vec2 p ) 
{ 
    p = vec2( dot(p, vec2(327.1, 111.7)), dot(p, vec2(269.5,183.3))); 
    return fract(cos(p) * movement); 
}
...
{
    float f = voronoi2D(p.xz) + fbm4(p.xz) + p.y * col1 * 50;
    
    f = mix( f, f * f * f * 0.5, f * abs(p.y));
    
    vec3 col = vec3(0.0);
    
    col = mix( vec3(freq1, 0.0, 0.4), vec3(0.0, 0.0, 2.0), f * f);
    
    // Ridges color animation.
    col = mix( col, vec3( 1.74, .95, 0.0), 2.5 * smoothstep(0.2, 1.8, 2.5 * fbm4(p.xz) + 0.3 * sin(time) * sin(time)));
...
{
    float fogAmount = 0.025 * exp(sin(ro.y) * .0) * (1.0 - exp(-dist * rd.y * 0.5)) / rd.y;
    
    // We leave some minumum, to always have some horizon 'fire'.
    return mix(rgb, 0.25 * FogColor + FogColor * sin(time) * sin(time), fogAmount);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 ro = vec3(0.5, 0.75, 1. + iTime * 0.5);  // LIVE CODE ME!!
    vec3 lk = ro + vec3(0., -1.0, 2.);
    
    // FOV - Field of view.
    float FOV = 1.9 / 2.;  // LIVECODE ME!! 
    vec3 forward = normalize(lk - ro);
    vec3 right = normalize(vec3(forward.z, rotation * 5.0, -forward.x )); 
