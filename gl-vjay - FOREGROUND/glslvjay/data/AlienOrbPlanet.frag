...
#define RODSA 5.0
#define RODSB 2.0
#define CA vec3(freq1, freq2, freq3)
#define CB vec3(freq4, 0.5, 0.5)
...
Scene march(vec3 ro, vec3 rd, float maxt) {
    ...
    for(int i = 0; i < 128; i++) {
        ...
        lat = (5.0 / (5.0 + lat * lat * 0.000001)) * step(3.8, rp.y);;
        rala += 0.3 / (5.0 + scene.rala * scene.rala * 1.0) * lat;
...
Scene traceScene(vec3 ro, vec3 rd, float maxt) {
    ...
    float rt = mod(T * 80.0, 1600.0) - 200.0;
    for (int i = 0; i < 2; i++) {
        vec4 ct = iCapsule( ro, rd, vec3(50.0 + float(i) * 30.0, 10.0, cro.z + rt), normalize(vec3(0.0, 0.0, 0.0)), RR, 200.0);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        //large spheres
        float r = rand(vec2(120.0, dt - 20.0));
        ...
        sphere = vec4(4.0 + r * 320.0, 10.0, dt - 5.0, 112.);
        if (r > 0.75) {
            vec2 si = sphIntersect(ro, rd, sphere);
            if (si.x > 0.0 && si.x < mint) {
                mint = si.x;
        ...
        sphere = vec4(-4.0 - r * 30.0, 10.0, dt - 25.0, 25.);
        if (r > 0.7) {
            vec2 si = sphIntersect(ro, rd, sphere);
            if (si.x > 0.0 && si.x < mint) {
                mint = si.x;
...
vec3 colourScene(vec3 ro, vec3 rd, Scene scene) {
    ...
    float spec = pow(max(dot(reflect(-ld, scene.n), -rd), 0.0), lightning * 5.0);
    float atten = 5.0 * -1.0 / (- + lt * lt * 0.0002);
    
    if (scene.id == TERRAIN_FLOOR) {
        
        pc = vec3(6.0, 0.05, 0.02) * 0.4 * diff;
        ...
        atten = 1.0 / (3.0 + lt * lt * 0.00001);
        pc = vec3(1.3, 1.0, 0.0) * atten;
        ...
        pc += vec3(0.05, 0.0, 0.3) * max(scene.n.z * -5.0, 0.0) * 0.02;
        pc += vec3(0.05, 0.0, 0.3) * max(scene.n.x, 0.0) * 0.02;
        pc += vec3(1.0) * spec * 0.1; // DEBUG
        pc *= atten;
...
void setupCamera(vec2 uv, inout vec3 rd) {
    ...
    vec3 right = normalize(vec3(forward.z, rotation, -forward.x)); 
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (scene.t < mint) {
        ...
        if (scene.id == TERRAIN_FLOOR) {
            pc = mix(colourScene(cro, rd, scene), pc, 1.3 / (1.0 + rp.y - terrainDepth)); 
            n = bump(rp * 0.6 + T, n);
        } else {
    ...
    fragColor = vec4(sqrt(clamp(pc, 0.0, 0.9)), 1.0);
}
