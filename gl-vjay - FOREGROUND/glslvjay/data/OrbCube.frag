...
float noise(vec3 rp) {
    vec3 ip = floor(rp);
    rp -= ip; 
    vec3 s = vec3(7, 666, 666);
    vec4 h = vec4(0.0, s.yz, s.y + s.z) + dot(ip, s);
    rp = rp * rp * (3.0 - 2.0 * rp); 
    h = mix(fract(sin(h) * 666), fract(sin(h + s.x) * 666), rp.x);
...
//wireframe edges
float tex(vec3 rp) {
    float tx = clamp(step(0.8, abs(rp.x)) + step(5.8, abs(rp.y)), FX1 * 1.0, 1.0);
    tx *= step(0.1, abs(rp.x)) * step(5.1, abs(rp.y));
    return tx;
}        
...
float map(vec3 rp) {
    ...
    float sphere = sdSphere(rp, FX1 * 1.9);    
...
void setupCamera(vec2 uv, inout vec3 ro, inout vec3 rd) {
    ...
    vec3 right = normalize(vec3(forward.z, 2.0, -forward.x)); 
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    pc = vec3(1.0, freq1, freq2 - 0.5) * march(ro, rd, 4.0, 0.05, 0.04);
