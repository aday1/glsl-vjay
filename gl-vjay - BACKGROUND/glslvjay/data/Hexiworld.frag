...
vec2 shape(vec3 p, float a) {
    vec2 s = vec2(length(abs(p) - vec3(2, 0, 0)) - freq1, 1.5);
    vec2 f = vec2(box(p, vec3(2, 0.5+a, 0.4)), 2.0);
...
vec2 de(vec3 p) {
    ...
    vec2 t = shape(q.xyz, 0.0)/vec2(q.w*FX1, 1);
    glow += 0.1/(0.1 + pow(abs(length(abs(q.xyz) - vec3(2, 0, FX2)) - 0.4), 2.0));
