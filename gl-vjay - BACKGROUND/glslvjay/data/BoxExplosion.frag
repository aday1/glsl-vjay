...
float deMengerSponge2(vec3 p, vec3 offset, float scale) {
    ...
    return (length(max(abs(z.xyz) - vec3(FX1, FX2 * 5.0, 1.0), 0.0))) / z.w;
}



float de(vec3 p) {
	p.xy *= rot(time); // CUBE ROTATION
	p.yz *= rot(time); // CUBE ROTATION
    return deMengerSponge2(p, vec3(FX2 +1.0), 2.0); // FX !!!
}
...
float dist(vec3 p){
	...
	p.xy = vec2(2.0);
		...
		if (HideStars==0.0)
	p.xyz *= rot3D(0.0); // WORLD ROTATE

		if (HideStars==1.0)
	p.xyz *= rot3D(time); // WORLD ROTATE
...
void main( void ) {
	...
	if(abs(d.x)<0.001){
		...
		if(abs(d.y)<0.001){
		gl_FragColor = vec4(vec3(diff*ray*cos(time)+0.25),1.0); // CUBE COLOR COL1 COL2 or FREQ1 or FREQ2
		}
	}else{
